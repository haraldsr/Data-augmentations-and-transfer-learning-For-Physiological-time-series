...
class CNNAttnModel(nn.Module):
    def __init__(self, dropout, nodes, numfeats, R_U=False, kernelsize=[3], numLayers=1, attention=True):
        super(CNNAttnModel, self).__init__()
        self.attention = attention
        self.cnn_layers = nn.Sequential(
            # Defining a 2D convolution layer
            nn.Conv1d(numfeats, nodes, kernel_size = kernelsize[0], padding=kernelsize[0]//2),
            nn.BatchNorm1d(nodes),
            nn.LeakyReLU(),
            nn.MaxPool1d(kernel_size=kernelsize[0], stride=2, padding=1),
            #nn.AdaptiveMaxPool1d(output_size=nodes),
            nn.Dropout(p=dropout)
        )
        n_h = numLayers
        if (n_h) % 2 == 0:
            # If the length is even, adjust it to the nearest odd number
            n_h += 1
            middle = n_h // 2
    ...
    def _add_layer(self, nodes: int, dropout: float, kernelsize: int, layer_index: int):
        self.cnn_layers.add_module("conv_" + str(layer_index), nn.Conv1d(nodes, nodes, kernel_size = kernelsize, padding=kernelsize//2))
        ...
        self.cnn_layers.add_module("maxpool_" + str(layer_index), nn.MaxPool1d(kernel_size=kernelsize, stride=2, padding=1))
...
class LinearS(nn.Module):
    ...
    def forward(self, out):
        #cont_emb = torch.cat(out, -1)
        op = self.batchnorm(self.firstLinear(out))
        op = self.LReLU(op)
        op = self.finalLinear(op).squeeze(-1)
...
class AutoencoderS(nn.Module):
    def __init__(self, dropout, nodes_before_last_layer, NumFeats, config=None):
        super(Autoencoder, self).__init__()


        kernel_size = config.CNNKernelSize
        self.freqmodels = torch.nn.Sequential(
            torch.nn.Conv1d(NumFeats, 8, kernel_size=kernel_size, padding=kernel_size//2),
            torch.nn.BatchNorm1d(8),
            torch.nn.LeakyReLU(),
            torch.nn.MaxPool1d(kernel_size=kernel_size, stride=2, padding=0),
            torch.nn.Dropout(p=dropout),  # Replace with your actual dropout value
            torch.nn.Conv1d(8, 16, kernel_size=kernel_size, padding=kernel_size//2),
            torch.nn.BatchNorm1d(16),
            torch.nn.LeakyReLU(),
            torch.nn.MaxPool1d(kernel_size=kernel_size, stride=2, padding=0),
            torch.nn.Dropout(p=dropout),  # Replace with your actual dropout value
            torch.nn.Conv1d(16, 32, kernel_size=kernel_size, padding=kernel_size//2),
        ...
        self.linear = Linear(32, nodes_before_last_layer, Out_Nodes)
    
    def forward(self, data):
        out = self.freqmodels(data)
...
class ClassificationS(nn.Module):
    def __init__(self, trained_model, nodes_before_last_layer, NumClasses):
        super(Classification, self).__init__()
        self.freqmodels = trained_model.freqmodels
        self.linear = Linear(32, nodes_before_last_layer, NumClasses)
...
def getModelS(config: object, trained_model = None, NumClasses: int = 1, nodes_before_last_layer: int = 500, classification: bool = True) -> object:
    """
    Get the model based on the given configuration.
    ...
    """
    if trained_model is None:
        model = AutoencoderS(config.dropout, nodes_before_last_layer=config.nodes_before_last_layer, NumFeats=config.NumFeats, config=config)
    else:
        if classification:
            model = ClassificationS(trained_model, nodes_before_last_layer=nodes_before_last_layer, NumClasses=NumClasses)
            #model = Classification(trained_model, nodes_before_last_layer=int(sum(config.nodes_per_freq)/2), NumClasses=NumClasses)
        else:
            model = Per_Sub_selfsup(trained_model, nodes_before_last_layer=nodes_before_last_layer, NumClasses=NumClasses)
