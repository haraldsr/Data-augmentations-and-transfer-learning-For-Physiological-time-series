...
class E4Data_freq(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        Returns:
            list: Tuple with lists of items at the specified index.
        '''
        try:
...
class E4Data_freq_stratify(torch.utils.data.Dataset):
    
    def __init__(self, data: list, device: torch.device, data_op=None, data_user=None):
        '''
        Initialize the E4Data_freq class.
        ...
        Args:
            ...
            device (torch.device): The device to be used.
            data_op (torch.tensor, List): The target data to be loaded.
            data_user (torch.tensor, List): The user data to be loaded.
        '''
        self.data = data
        self.labels = data_op
        ...
        # Create a mapping between original user IDs and contiguous indices
        self.unique_users, user_mapping = torch.unique(self.users, return_inverse=True)
        
        # Calculate the maximum number of data points from any user
        self.max_data_points = max([len(self.data[0][self.users == user]) for user in self.unique_users])
    ...
    def __len__(self) -> int:
        ...
        Returns:
            int: The length of the dataset.
        '''
        return self.max_data_points * len(self.unique_users)
    ...
    def __getitem__(self, index):
        ...
        '''
        try:
            user_index = index % len(self.unique_users)  # Get the user index
            user_id = list(self.unique_users)[user_index]  # Get the user ID
            
            # Get indices for the user's data points
            user_indices = [i for i, uid in enumerate(self.users) if uid == user_id]
            
            # Upsample data points for the user if needed
            while len(user_indices) < self.max_data_points:
                user_indices += user_indices
            
            # Select data points for the current index
            selected_indices = user_indices[index // len(self.unique_users)]
            
            out = [d[selected_indices, :, :].to(self.device) for d in self.data]
            if self.labels is None:
                return out
            elif self.users is None:
                return out, self.labels[selected_indices].to(self.device)
            else:
                return out, self.labels[selected_indices].to(self.device)#, self.users[selected_indices].to(self.device)
        except IndexError:
            raise IndexError("Invalid index")
...
def get_freq_single(X_data: list, duration: int) -> list:
    ...
    # if torch.backends.mps.is_available():
    X_data_freq = [torch.tensor(X_data_4).float(), torch.tensor(X_data_64).float()]
    # else:
